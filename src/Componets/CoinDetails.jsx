...
const CoinDetails = () => {
  ...
  const [chatArray, setChartArray] = useState([])
        ...
        loading ? (<Loader />) :
          <>
            ...
            <HStack p={'5'} overflowX={'auto'}>
              {
                btns.map((i) => (
                  <Button key={i} onClick={()=>btnSwitch(i)}>{i}</Button>
                ))
              }
            </HStack>
            ...
            <VStack p={'16'} spacing={'4'} alignItems={'flex-start'}>
              ...
              <Badge p={'2xl'} bgColor={'blackAlpha.800'} color={'white'}>{`#${coin.market_cap_rank}`}</Badge>
...
const Item = ({ title, value }) => (
  <HStack w={'full'} justifyContent={'space-between'} my={3}>
